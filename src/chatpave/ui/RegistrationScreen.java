/*
 * Located in: src/chatpave/ui/RegistrationScreen.java
 */
package chatpave.ui;
 ...
 */
public class RegistrationScreen extends Canvas implements CommandListener, Runnable {

    // --- State Management ---
    private static final int FOCUS_USERNAME = 0;
    private static final int FOCUS_EMAIL = 1;
    private static final int FOCUS_PASSWORD = 2;
    private static final int FOCUS_REGISTER_BUTTON = 3;
    private static final int FOCUS_BACK_BUTTON = 4;
    ...
    // --- UI Color Palette (same as LoginScreen) ---
    ...
    protected void paint(Graphics g) {
        ...
        drawAppBar(g, "Create Account");

        if (isLoading) {
            g.setColor(COLOR_TEXT_DARK);
            g.drawString("Registering...", width / 2, height / 2, Graphics.HCENTER | Graphics.BASELINE);
            return;
        }

        int contentY = APP_BAR_HEIGHT + 20;
        ...
        drawTextField(g, "Email", email, fieldX, contentY + 70, fieldWidth, fieldHeight, currentFocus == FOCUS_EMAIL);
        drawTextField(g, "Password", password, fieldX, contentY + 140, fieldWidth, fieldHeight, currentFocus == FOCUS_PASSWORD);
        
        if (statusMessage.length() > 0) {
            g.setColor(0xFF0000);
            g.drawString(statusMessage, width / 2, contentY + 200, Graphics.HCENTER | Graphics.TOP);
        }
        ...
        drawButton(g, "Back", (width / 2) - buttonWidth - 10, buttonY, buttonWidth, buttonHeight, currentFocus == FOCUS_BACK_BUTTON);
    ...
    protected void keyPressed(int keyCode) {
        ...
        if (gameAction == FIRE) {
            ...
            currentFocus = (currentFocus == 0) ? 4 : currentFocus - 1;
        } else if (gameAction == DOWN) {
            currentFocus = (currentFocus + 1) % 5;
        } else if (gameAction == LEFT) {
            if (currentFocus == FOCUS_REGISTER_BUTTON) currentFocus = FOCUS_BACK_BUTTON;
        } else if (gameAction == RIGHT) {
            if (currentFocus == FOCUS_BACK_BUTTON) currentFocus = FOCUS_REGISTER_BUTTON;
        }
        repaint();
    }
    ...
    /**
     * This method now correctly handles the boolean return type from ApiClient.register().
     */
    public void run() {
        boolean registrationSuccess = ApiClient.register(username.toString(), email.toString(), password.toString());
        isLoading = false;
    ...
    private void handleFireAction() {
        if (currentFocus == FOCUS_REGISTER_BUTTON) {
            isLoading = true;
            statusMessage = "";
            repaint();
            new Thread(this).start();
        } else if (currentFocus == FOCUS_BACK_BUTTON) {
    ...
    private void showCurrentTextBox() {
        if (currentFocus == FOCUS_USERNAME) {
            editingField = FOCUS_USERNAME;
            showTextBox("Enter Username", username, TextField.ANY);
        } else if (currentFocus == FOCUS_EMAIL) {
            editingField = FOCUS_EMAIL;
            showTextBox("Enter Email", email, TextField.EMAILADDR);
    ...
    public void commandAction(Command c, Displayable d) {
        if (c == okCommand) {
            ...
            } else if (editingField == FOCUS_EMAIL) {
                email.setLength(0);
                email.append(text);
